...
@php
    ...
    $img = $imagem ?? '/placeholder-card.png';
@endphp

<div {{ $attributes->merge(['class' => 'bg-white rounded-xl shadow-md overflow-hidden']) }}>
    <div class="px-4 py-3 border-b">
        <h4 class="text-lg font-semibold text-main">{{ $nome }}</h4>
    </div>

    <div class="p-4">
        <div class="rounded-lg overflow-hidden mb-3 bg-gray-100">
            <img src="{{ $img }}" alt="{{ $nome }}" class="w-full object-cover h-40" />
        </div>
        ...
        <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-4 text-sm text-gray-700">
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-medium">{{ $est >= 0 ? '+' . $est : $est }}</span>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-medium">{{ $din >= 0 ? '+' . $din : $din }}</span>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-medium">{{ $rep >= 0 ? '+' . $rep : $rep }}</span>
                </div>
            </div>
        </div>
